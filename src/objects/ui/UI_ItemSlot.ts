...
export default class ItemSlot extends Phaser.GameObjects.NineSlice {
        ...
        this.setVisible(false)
            ...
            .on("pointerdown", (pointer: Phaser.Input.Pointer) => {
                ...
                if (pointer.leftButtonDown() && held.Type === "Item" ) {
                    ...
                    // Do not allow drops on slots that already have an item
                    if ( GD.Inventory[this.InventoryIndex] !== undefined && GD.Inventory[this.InventoryIndex] !== null ) {
                        return;
                    }
                    ...
                    this.scene.Game.ActionManager.UseItem(this.DataInventorySlot.ID);
    ...
    private updateItemTexture(): void {
        if (this.DataInventorySlot) {
            const baseItemData = ItemData[this.DataInventorySlot.ID];
