...
export default class BuildingHelper {
    ...
    ActivateBuildingMode ( Building: string ) : void {
        ...
        this.Placeholder.setActive(true).setVisible(true).setSize(BuildingData[Building].Size.Width - 1, BuildingData[Building].Size.Height - 1);
        this.UI.BuidlingPlacementModeHelpText.setVisible(true);
        this.UI.TownManagementPanel.Hide();
        this.UI.TownManagementPanel.BuildingList.clear(true, true);
    ...
    DeactivateBuildingMode () : void {
        ...
        this.UI.TownManagementPanel.Show();
    ...
    CheckIfPlacementValid () {
        ...
        // Check if placeholder body overlaps with bodies of other buildings
        let overlappingBuilding = null;
        this.scene.Buildings.getChildren().forEach( (building: Building) => {
            if ( Phaser.Geom.Intersects.RectangleToRectangle(this.Placeholder.getBounds(), building.getBounds()) ) {
                overlappingBuilding = building;
            }
        });

        if ( this.scene.SelectedBuilding != "Town Centre" ) {
            if ( overlapping == undefined && isContained && overlappingBuilding == null ) {
                console.log("Valid placement for " + this.scene.SelectedBuilding);
                this.Placeholder.setFillStyle(0x44a617, 0.8);
                this.ValidPlacement = true;
            } else {
                console.log("Invalid placement for " + this.scene.SelectedBuilding);
                this.Placeholder.setFillStyle(0xdb382c, 0.8);
                this.ValidPlacement = false;
            }
        } else {
            if ( overlapping == undefined && overlappingBuilding == null ) {
